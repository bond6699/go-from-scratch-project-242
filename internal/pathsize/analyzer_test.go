...
func TestAnalyzer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"test1", Create(false, false, false),
			filepath.Join(root, "tests", "test1"), 25318592,
		},
		{
			"test2 | Without recursive", Create(false, false, false),
			filepath.Join(root, "tests", "test2"), 2077016,
		},
		{
			"test2 | Recursive", Create(true, false, false),
			filepath.Join(root, "tests", "test2"), 7304971,
		},
		{
			"test3 | Without recursive, hidden", Create(false, false, false),
			filepath.Join(root, "tests", "test3"), 7899980,
		},
		{
			"test3 | Recursive, Hidden", Create(true, true, false),

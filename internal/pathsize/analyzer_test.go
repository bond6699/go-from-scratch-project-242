...
func TestAnalyzer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"pathsize/Analyze() test5 | Folder with recursive and hidden files ( Recursive true, All true )",
			CLIArgs{true, true, false},
			filepath.Join(root, "tests", "test3"), 12745409,
		},
		{
			"pathsize/Analyze() test6 | File path ( Default => All true, Recursive false )",
			CLIArgs{false, true, false},
			filepath.Join(root, "tests", "test3", "UserGameStats_783965222_220.bin"), 38,
		},
		{
			"pathsize/Analyze() test6 | Hidden File path ( Default => All true, Recursive false )",
			CLIArgs{false, true, false},

...
func TestAnalyzer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"test1 Default ( Recursive false, All false )",
			CLIArgs{false, false, false},
			filepath.Join(root, "tests", "test1"), 25318592,
		},
		{
			"test2 | Folder without recursive ( Recursive false, All false ) ",
			CLIArgs{false, false, false},
			filepath.Join(root, "tests", "test2"), 2077016,
		},
		{
			"test3 | Folder with recursive ( Recursive true, All false )",
			CLIArgs{true, false, false},
			filepath.Join(root, "tests", "test2"), 7304971,
		},
		{
			"test4 | Folder without recursive, with hidden files ( Recursive false, All true )",
			CLIArgs{false, false, false},
			filepath.Join(root, "tests", "test3"), 7899980,
		},
		{
			"test5 | Folder with recursive and hidden files ( Recursive true, All true )",
			CLIArgs{true, true, false},
			filepath.Join(root, "tests", "test3"), 12745409,
		},
		{
			"test6 | File path ( Default => All true, Recursive false )",
			CLIArgs{false, true, false},
			filepath.Join(root, "tests", "test3", "UserGameStats_783965222_220.bin"), 38,
		},
		{
			"test6 | Hidden File path ( Default => All true, Recursive false )",
			CLIArgs{false, true, false},
